# Vibe Code Game - Godot 4.5 Rhythm RPG

A **Godot 4.5 Rhythm RPG** developed using a two-tier AI workflow with 29 AI-generated prompts for distributed development.

## 🎮 Project Overview

This is a complete Rhythm RPG combining:
- **Pokemon**-style monster collection
- **Zelda**-like real-time combat
- **Lufia 2**-inspired puzzle mechanics
- **Rhythm game** mechanics integrated throughout

**26 game systems** (S01-S26) implemented via multi-agent AI development.

## 🔄 Two-Tier AI Development Workflow

This project uses a unique **Claude Code Web → Godot MCP** handoff approach:

### Tier 1: Claude Code Web (File Creation)
- Creates all GDScript (.gd) files with complete implementations
- Creates all JSON data files
- Generates HANDOFF-[system-id].md with scene configuration instructions
- **Cannot** use Godot MCP (web version limitation)

### Tier 2: Godot MCP Agents (Scene Configuration)
- Reads HANDOFF-[system-id].md from Tier 1
- Uses GDAI MCP tools: `create_scene`, `add_node`, `attach_script`, `update_property`
- Tests in Godot editor: `play_scene`, `get_godot_errors`
- Runs quality gates and integration tests

**Why this works:** Claude Code Web excels at code generation; Godot MCP agents handle Godot-specific operations. This maximizes efficiency by leveraging each tier's strengths.

---

# Meta-Prompting System for Claude Code

*Below is the original meta-prompting documentation this project was built with.*
